/*******************************************************************************
 *
 *  @file simulation-mode-three-sensor-line-follow.js Test the robot two sensor
...
/**
 * Class Simulation.Mode.LineFollowTwoSensor is a simulation mode that sets up
 * a robot with two light sensors and allows it to loop around a room.  UI
 * elements are set up so that the user can change the parameters controlling
 ...
 */
Simulation.Mode.LineFollowThreeSensor = class extends Simulation.Mode.ModeType {
    static staticName = "LineFollowThreeSensor";
    /**
     * The constructor that sets up the simulation variables
     */
    constructor() {
        super();
        this.name = "LineFollowThreeSensor";
        this.uiDivID = "simulation-mode-line-follow-three-sensor";
        this.setupUI();
    ...
    addNewUIElements() {
        ...
        if(document.getElementById("sm-lf3s-sensor-separation-slider").children.length) {
            document.getElementById("sm-lf3s-show-grid").children[0].remove();

            document.getElementById("sm-lf3s-sensor-separation-slider").children[0].remove();
            document.getElementById("sm-lf3s-sensor-separation-val").children[0].remove();

            document.getElementById("sm-lf3s-sensor-radius-slider").children[0].remove();
            document.getElementById("sm-lf3s-sensor-radius-val").children[0].remove();

            document.getElementById("sm-lf3s-pd-rotation-kp-slider").children[0].remove();
            document.getElementById("sm-lf3s-pd-rotation-kp-val").children[0].remove();

            document.getElementById("sm-lf3s-pd-rotation-kd-slider").children[0].remove();
            document.getElementById("sm-lf3s-pd-rotation-kd-val").children[0].remove();

            document.getElementById("sm-lf3s-pd-vel-slider").children[0].remove();
            document.getElementById("sm-lf3s-pd-vel-val").children[0].remove();

            document.getElementById("sm-lf3s-load-room-json-button").children[0].remove();
        }

        // show grid check box
        this.showGridCheckbox = createCheckbox("Draw Grid Lines", true);
        this.showGridCheckbox.parent("sm-lf3s-show-grid");

        // sensor separation
        this.sensorSeparationSlider = createSlider(0, 1, this.sensorSeparation, 0.05);
        this.sensorSeparationSlider.parent("sm-lf3s-sensor-separation-slider");

        this.sensorSeparationDisplay = createP();
        this.sensorSeparationDisplay.parent("sm-lf3s-sensor-separation-val");
        ...
        this.sensorRadiusSlider.parent("sm-lf3s-sensor-radius-slider");

        this.sensorRadiusDisplay = createP();
        this.sensorRadiusDisplay.parent("sm-lf3s-sensor-radius-val");
        ...
        this.rotationKpSlider.parent("sm-lf3s-pd-rotation-kp-slider");

        this.rotationKpDisplay = createP();
        this.rotationKpDisplay.parent("sm-lf3s-pd-rotation-kp-val");
        this.rotationKpDisplay.elt.innerText = "Kp: " + str(this.rotationKp);

        // pd rotation kd
        this.rotationKdSlider = createSlider(0, 50, this.rotationKd, 0.25);
        this.rotationKdSlider.parent("sm-lf3s-pd-rotation-kd-slider");

        this.rotationKdDisplay = createP();
        this.rotationKdDisplay.parent("sm-lf3s-pd-rotation-kd-val");
        ...
        this.velSlider.parent("sm-lf3s-pd-vel-slider");

        this.velDisplay = createP();
        this.velDisplay.parent("sm-lf3s-pd-vel-val");
        this.velDisplay.elt.innerText = "Vel: " + str(this.algorithmForwardVel);

        this.loadRoomButton = UI.createButton("Upload Room", "sm-lf3s-load-room-json-button", Simulation.Mode.LineFollowThreeSensor.loadRoom);
    }
    ...
    setupRobot() {
        ...
        const algorithm = new Robot.Algorithm.ThreeSensorFollow(
...
Simulation.Mode.ModeList.push(Simulation.Mode.LineFollowThreeSensor);

Simulation.Mode.LineFollowThreeSensor.loadRoom = async function() {
